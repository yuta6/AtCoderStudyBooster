...
atcdr -d B 223
でB問題の223をダウンロードしてくれる.

atcdr -d 223 B
で223ディレクトリーをつくってBをダウンロードしてくれる.

atcdr -d B 223..230
でB問題の223から230をダウンロードできる.
各々のディレクトリーでgit initしてくれる?
...
対象ディレクトリーまで移動後にそこで...
    ...
    atcdr -t xxx.py
    でテストできる. 自動コミット機能もつける?
    テストケースが通ったら自動でコミットしてくれる? 通ったときの実行時間などとともに?

    atcdr -t 
    の場合, 適当に選んだファイルの1つをテストする.

    -g, --generate
    
    atcdr -g Python
    AIで解答例を生成する. chatGPToのAPIかseleniumでログインする. 
    
    -s, --submit 

    atcdr -s xxx.py
    で提出する. テストケースで自動でテストして, それらがすべてパスした場合のみ提出する.
    HTMLのURLにアクセスして, 言語を選択して, Submitボタンを押す.

    -j, --judge　[すべてのケースが公開されている場合のため]
    atcdr -j xxx.py
    対象ディレクトリーでジャッジできる.
