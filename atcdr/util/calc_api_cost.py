...
def count_tokens(text: str) -> None:
    ...
    print(
        f"{gpt4o_model.name} : {len(gpt4o_model.encode(text))}, {gpt4_model.name} : {len(gpt4_model.encode(text))}"
    )
...
class CalcApiCost:
    ...
    GPT4OMINI_INPUT_RATE = 0.15 / 1000**2
    GPT4OMINI_OUTPUT_RATE = 0.60 / 1000**2

    COST_TYPE_INPUT = "input"
    COST_TYPE_OUTPUT = "output"

    def __init__(self, text: str, cost_type: str, model_name: str) -> None:
        self._text = text
    ...
    def cost_type(self, new_cost_type: str):
        if new_cost_type not in [self.COST_TYPE_INPUT, self.COST_TYPE_OUTPUT]:
            raise ValueError(
                "コストタイプの指定が間違っています. 'input'か'output'で指定してください."
            )
    ...
    def _calculate_cost(self):
        ...
        if self._cost_type == self.COST_TYPE_INPUT:
            cost_usd = self._token_count * self.GPT4O_INPUT_RATE
            cost_usd = self._token_count * self.GPT4O_OUTPUT_RATE
        elif self._cost_type == self.COST_TYPE_OUTPUT:
            cost_usd = self._token_count * self.GPT4OMINI_INPUT_RATE
            cost_usd = self._token_count * self.GPT4OMINI_OUTPUT_RATE
...
def main():
    ...
    """
    print(CalcApiCost(text, CalcApiCost.COST_TYPE_INPUT))
